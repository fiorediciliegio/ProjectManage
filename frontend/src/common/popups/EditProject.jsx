...
export default function EditProject({ project, onClose, onUpdated, onError }) {
  ...
  useEffect(() => {
    ...
    setProjectInfo({
      ...
      pjnumber: project.pjnumber || "",
      pjmanager: project.pjmanager || "",
      pjmanager_id: "",
      pjtype: project.pjtype || "",
      pjvalue: project.pjvalue || "",
      pjcurrency: project.pjcurrency || "",
      pjstart_date: project.pjstart_date || "",
      pjend_date: project.pjend_date || "",
      pjaddress: project.pjaddress || "",
      pjdescription: project.pjdescription || "",
    });
  }, [project]);
    ...
    const fetchProjectManagers = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          const currentManager = managers.find((manager) => manager.pername === project?.pjmanager);
          if (currentManager) {
            setProjectInfo((prev) => ({ ...prev, pjmanager_id: currentManager.perid }));
          }
        }
      } catch (err) {
        if (isMounted) {
  ...
  const onSubmit = async () => {
    ...
    try {
      const response = await axios.patch(`http://localhost:8000/projects/${project.pjid}/`, projectInfo);
      onUpdated(response.data?.data?.project || null);
      onClose();
    } catch (err) {
      ...
      if (backendNumberError || backendManagerError) {
        ...
        onError(responseData?.message || "你没有权限修改项目详情");
      } else {
  ...
  return (
    <CommonCreate title="修改项目详情" fields={fields} onClose={onClose} onSubmit={onSubmit}>
      {loading && <p>正在保存...</p>}
